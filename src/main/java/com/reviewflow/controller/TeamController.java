...
public class TeamController {
    ...
    public ResponseEntity<ApiResponse<List<TeamResponse>>> autoAssign(
        ...
        List<Team> created = teamService.autoAssignTeams(assignmentId, user.getUserId(), maxSize);
    ...
    public ResponseEntity<ApiResponse<TeamResponse>> get(
            ...
            @AuthenticationPrincipal ReviewFlowUserDetails user) {
        Team team = teamService.getTeamByIdWithAccessControl(id, user.getUserId(), user.getRole());
    ...
    public ResponseEntity<ApiResponse<TeamResponse>> rename(
            ...
            @AuthenticationPrincipal ReviewFlowUserDetails user) {
        Team team = teamService.renameTeam(id, request.getName(), user.getUserId());
        return ResponseEntity.ok(ApiResponse.ok(toResponse(team)));
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> invite(
            ...
            @AuthenticationPrincipal ReviewFlowUserDetails user) {
        TeamMember member = teamService.inviteMember(id, request.getInviteeEmail(), user.getUserId());
        return ResponseEntity.ok(ApiResponse.ok(Map.of("teamMemberId", member.getId(), "status", member.getStatus().name())));
    }
    ...
    public ResponseEntity<ApiResponse<Map<String, String>>> respond(
            ...
            @AuthenticationPrincipal ReviewFlowUserDetails user) {
        TeamMember member = teamService.respondToInvite(id, request.getAccept(), user.getUserId());
    ...
    public ResponseEntity<ApiResponse<Map<String, String>>> removeMember(
            ...
            @AuthenticationPrincipal ReviewFlowUserDetails user) {
        teamService.removeMember(id, userId, user.getUserId());
    ...
    public ResponseEntity<ApiResponse<TeamResponse>> lock(
            @PathVariable Long id,
            @AuthenticationPrincipal ReviewFlowUserDetails user) {
        Team team = teamService.lockTeam(id);
        return ResponseEntity.ok(ApiResponse.ok(toResponse(team)));
    }
