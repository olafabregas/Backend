    ...
    public Submission upload(Long teamId, Long assignmentId, String changeNote,
        ...
        if (file.getSize() > submissionMaxFileSizeBytes) {
            ...
            throw new ValidationException(
                    "File size " + sizeMB + "MB exceeds maximum of " + maxMB + "MB",
                    "FILE_TOO_LARGE"
            );
        }
        ...
        try {
            ...
            String hashedOwnerId = submissionType == SubmissionType.INDIVIDUAL
                    ? hashidService.encode(uploaderId)
                    : hashidService.encode(teamId);
            String relativePath = S3KeyBuilder.submissionKey(
                    hashedAssignmentId,
                    hashedOwnerId,
                    nextVersion,
                    originalName);
